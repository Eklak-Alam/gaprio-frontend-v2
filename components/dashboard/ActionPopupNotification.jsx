...
export default function ActionPopupNotification({ onCountChange }) {
    ...
    const checkForNewActions = useCallback(async () => {
        try {
            const res = await api.get('/monitoring/actions', { params: { status: 'pending' } });
    ...
    const refreshCount = async () => {
        ...
        try { const r = await api.get('/monitoring/actions/count'); onCountChange(r.data.count || 0); } catch { /* silent */ }
    };
    ...
    const handleExecute = async () => {
        ...
        setExecuting(true);
        try { await api.post(`/monitoring/actions/${currentAction.id}/execute`); notifySW(currentAction.id); removeFirst(); await refreshCount(); }
    ...
    const handleIgnore = async () => {
        ...
        setDismissing(true);
        try { await api.post(`/monitoring/actions/${currentAction.id}/reject`); notifySW(currentAction.id); removeFirst(); await refreshCount(); }
